...
export default function Order() {
  ...
  const convertPrice = (price: number) => {
    return (price * currentCurrency.rate).toFixed(0);
  };

  useEffect(() => {
    if (Object.keys(cartItems).length === 0 && !isLoading && !isOrderCompleted) {
      router.push('/cart');
    }
  }, [cartItems, isLoading, router, isOrderCompleted]);
    ...
    try {
      ...
      products.forEach(({ product, quantity }) => {
        const orderItem = {
          ...
          totalPrice: product.price * quantity * currentCurrency.rate,
        };
      ...
      <div className={styles.content}>
        ...
        <div className={styles.orderSummary}>
            ...
            {products.map(({ product, quantity }) => (
              <div key={product.id} className={styles.summaryItem}>
                ...
                <div className={styles.itemPrice}>
                  {nf.format(Number(convertPrice(product.price)) * quantity)} {currentCurrency.symbol}
                </div>
              </div>
            ))}
          </div>
          ...
          <div className={styles.total}>
            <span>Итого:</span>
            <span>{nf.format(Number(calculateTotal()))} {currentCurrency.symbol}</span>
          </div>
