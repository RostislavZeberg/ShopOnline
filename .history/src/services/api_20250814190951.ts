...
export const fetchProductsWithCategories = async (): Promise<IProductTotal[]> => {
  ...
  return products.map(async (product: IProductBase) => {
    const productID = await getProductById(product.id)
    
    const category = categories.find((cat: { id: number; }) => cat.id === productID.category.id);
