...
export const fetchProductsWithCategories = async (): Promise<IProductTotal[]> => {
  ...
  // Преобразуем данные в простые объекты
  const products = JSON.parse(JSON.stringify(productsResponse.data.items));
  const categories = JSON.parse(JSON.stringify(categoriesResponse.data.items));

  return products.map((product: { categoryId: number; }) => {
    const category = categories.find((c: { id: number; }) => c.id === product.categoryId);
    return {
      ...product,
      category: category || {
        id: 0,
