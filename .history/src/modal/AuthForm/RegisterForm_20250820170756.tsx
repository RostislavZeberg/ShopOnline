...
export const RegisterForm: FC<RegisterFormProps> = ({ setAuthType }) => {
  ...
  const [userData, setUserData] = useLocalStorageAccount('userData', null);
  ...
  const userData = useLocalStorageAccount('userData', storedValue);
  let userData = null;
  ...
  if (formData.email === userData?.email && showForm) {
    return (
      <div className="">
        ...
        <button onClick={() => {setShowForm(false), formData.email = ''}}>Вернуться к регистрации</button>
      </div>
    )
  }
