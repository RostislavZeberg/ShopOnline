...
export const SearchProductDetailed = () => {
  ...
  // Ref для анимируемого элемента
  const formRef = useRef<HTMLFormElement>(null);
  const isOpenRef = useRef(isOpenSearchDetailed);
  // Флаг для отслеживания первого рендера
  const isFirstRender = useRef(true);
  // Ref для отслеживания статуса анимации
  const animationInProgress = useRef(false);

  useEffect(() => {
    isOpenRef.current = isOpenSearchDetailed;
  ...
  useEffect(() => {
    ...
    if (isFirstRender.current) {
      ...
      // Сразу скрываем форму при первой загрузке
      gsap.set(formRef.current, { display: 'none', opacity: 0, y: -270 });
      return;
    }
    ...
    const ctx = gsap.context(() => {
      if (isOpenSearchDetailed) {
        ...
        gsap.to(formRef.current, {
          ...
          duration: 1,
          onStart: () => {
            // Убедимся, что форма видима перед началом анимации скрытия
            gsap.set(formRef.current, { display: 'flex' });
          },
          onComplete: () => {
  ...
  return (
    <form
      ...
      style={{ display: 'none', opacity: 0, transform: 'translateY(-270px)' }}
    >
